...
SX126x::SX126x(SKM_SX1662_Interface* interface, SX126xConfig* config){
...
void SX126x::setPacketSize(uint8_t size){
	if(config->packetParams.GFSK.packetLength == PacketParams::PacketLength::Const){
...
void SX126x::configurate(){

	setModeSTDBY(SX126x::STDBY::RC);

	writePacketType(config->packetType);

	if(config->txClamp){
		writeTxClampEnable();
	}

	writeRegulatorMode(config->regulationMode);

	writeBufferBaseAddress(0, 0);

	writeModParams(config->modParams);

	writeFrequency(config->frequency);

	writeTxParams(config->txParams);

	writePacketParams(config->packetParams);
...
void SX126x::writePacketSize(uint8_t size){
	PacketParams packetParams = config->packetParams;
...
bool SX126x::radioExecSetCmd(Comm command, uint8_t* pBuffer, uint16_t Size){

	return interface->execSetCmd((uint8_t) command, pBuffer, Size);
}

bool SX126x::radioExecGetCmd(Comm command, uint8_t* pBuffer, uint16_t Size){

	return interface->execGetCmd((uint8_t) command, pBuffer, Size);
}

bool SX126x::radioWriteBuffer(uint8_t Offset, uint8_t* pBuffer, uint16_t Size){

	return interface->writeBuffer(Offset, pBuffer, Size);
}

bool SX126x::radioReadBuffer(uint8_t Offset, uint8_t* pBuffer, uint16_t Size){

	return interface->readBuffer(Offset, pBuffer, Size);
}

bool SX126x::radioWriteRegisters(Reg Address, uint8_t* pBuffer, uint16_t Size){

	return interface->writeRegisters((uint8_t) Address, pBuffer, Size);
}

bool SX126x::radioReadRegisters(Reg Address, uint8_t* pBuffer, uint16_t Size){
