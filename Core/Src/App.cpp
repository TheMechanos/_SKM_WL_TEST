...
void App::init(){
	...
	dev->radio.onRx([this](SKMPacket* packet){
		System::logLn("Source : %08X", packet->header.source);
		System::logLn("Sender : %08X", packet->header.sender);
		System::logLn("Destiny: %08X", packet->header.destiny);

		System::logLn("Hops : %d", packet->header.hops);
		System::logLn("Key  : %d", packet->header.cryptoKey);
		System::logLn("Type : %d", packet->header.type);
		System::logLn("ID   : %d", packet->header.id);


		System::logLn("Data size=%d", packet->getDataSize());
		System::logSt("Data: ");
		for (uint8_t q = 0; q < packet->getDataSize(); q++){
			System::log("%02x ", packet->dataIdx[q]);
		}
		System::log("\r\n");
